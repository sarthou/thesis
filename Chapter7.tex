...
\section[Generating Referring Expression]{Generating Referring Expression with compound relations}

\subsection{Determining a referring expression validity}

%\begin{remark}[Valid labels]
%Each time a child node is created with an empty set its family of sets, this means that a label of the CR is represented in its entirety at this node. The empty set should be removed to continue the process and the corresponding node could be marked as holding a valid label.
%\end{remark}
...
%\begin{remark}[Radix tree]
%A radix tree structure (also called compact prefix tree) could be used by merging each node that is the only child with its parent if the parent node does not represent a valid label. In the example of Fig.~\ref{fig:advanced_labels_graph}, the node with the property $p_2$ could be merged with its parent. We do not perform this optimization at this stage for implementation facilitation but we will consider it during the REG process.
%\end{remark}
