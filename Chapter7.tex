...
To represent the verbal link, we also choose to use parametric patterns, patterns for short, defined as labels in the ontology. A \acrshort{cr} can have multiple labels (i.e. patterns) depending on the subject of the pattern and the involved relations in the verbal link. The labels are not directly applied to the \acrshort{ce} but to the class it inherits. This means that all the entities inheriting from a class having its labels respecting\improvement{respecting what?} the pattern are \acrshort{ce}. In a way, we define here a relation-class but only through the use of labels.


\improvement{the "respecting" aspect of the textual definition is not present in the formal defintion.}
\improvement{Very confusing, you need to define what a pattern is (formally) first.}
...
\end{theorem}
\improvement{Doesn't Lt contain labels that are not patterns ? In this case, this definition would misclassify normal entitites as compound entities.}
...
\improvement{Je trouve que c'est un choix criticable de remplacer des entités et relations claires ("hasBuyer", ...) par des trucs complétements abstraits (p3, ...).}
\improvement{before theorem 7}
...
\improvement{you need to given an example here.}
...
\improvement{très peu clair et sans transition avec la partie précédente. L'objectif de ces mini introduction est vraiment d'être limpide pour que le lecteur sache où on l'emmene, même si il a du mal à s'accrocher au reste (corollaire: faut pas supposer qu'il ait une compréhension fine du reste). Pas clair principalement parce que tu met la charue avant les boeufs : tu expliques qu'il faut optimiser l'algo que tu n'as pas encore présenté.}
...
\improvement{Tu ne présentes pas la stratégie. (et ça rend toute la suite pas claire).}
...
\improvement{Pas clair pourquoi c'est ton but... Mon intuition c'est que tu crées des sous arbres que tu vas greffer à ton arbre de recherche "de base". Les contraintes qui en découlent sont qu'on ajoute un propriété à la fois et que les noeud terminaux (dans la greffe) doivent être complet (verbalisable).}
...
\begin{enumerate}
	...
	\item The width of the tree must be as small as possible. \improvement{Not a constraint, but an optmisation criteria that is hard to justify with talking about how it will be exploited.}
\end{enumerate}
