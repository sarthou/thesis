...
Here we have shown that the exploration of \acrshort{cr} has an impact, even if being acceptable with regard to \acrshort{hri} applications. However, the major advantage of this solution is that in case no \acrshort{cr}s are described in the \acrshort{kb}, no extra exploration is needed. This is a difference with the previous solution (see Chapter~\ref{chap:6}) where even if no tasks were described, the algorithm was trying to find some at each step. This means that, unlike the previous solution, if no \acrshort{cr}s are described in the \acrshort{kb}, the current solution has the same performance as the original version.
